...
// vmx128-fuzz sets this when vmx128_fuzz_rm_pass requests a fixed RN (-1 =
// default).
...
// VX-form AltiVec (primary opcode 4). `xo_low11` matches the low 11 bits used
// in `ppc_opcode_table_gen.cc` (e.g. vadduhm = 0x040).
...
class TestGuestPpcBlock {
 public:
  ...
  void Run(const std::vector<uint32_t>& guest_instructions,
           ...
           uint32_t entry_pc = 0x80000000, int rounding_mode = -1);
