...
void ProfileConfigDialog::OnDraw(ImGuiIO& io) {
  ...
  if (selected_xuid_) {
    ProfileBackup backup(emulator_window_->emulator()->content_root(),
                         emulator_window_->emulator()->storage_root() /
                             "cache" / "profile_backups");
    if (ImGui::Button("Backup selected profile")) {
      std::string error;
      if (!backup.ExportProfile(selected_xuid_, error)) {
        new ui::HostNotificationWindow(emulator_window_->imgui_drawer(),
                                       "Profile backup failed", error, 0);
      }
    }
  }
