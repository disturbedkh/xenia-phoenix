...
#include "xenia/kernel/netplay/netplay_obs.h"
...
void XLiveAPI::Init() {
  ...
  if (cvars::network_mode == NETWORK_MODE::OFFLINE) {
    ...
    netplay_obs::Emit("api_offline", "network_mode=offline");
    return;
  }
  ...
  if (!IsConnectedToServer()) {
    ...
    initialized_ = InitState::Failed;
    netplay_obs::Emit("api_connect_failed", GetApiAddress());
    return;
  }
  ...
  netplay_obs::Emit("api_connected", OnlineIP_str());
...
void XLiveAPI::QoSPost(uint64_t sessionId, uint8_t* qosData, size_t qosLength) {
  ...
  netplay_obs::Emit("qos_posted",
                    fmt::format("{:016X}:{}", sessionId, qosLength));
}
...
void XLiveAPI::DeleteSession(uint64_t sessionId) {
  netplay_obs::Emit("session_deleted", fmt::format("{:016X}", sessionId));
...
void XLiveAPI::XSessionCreate(uint64_t sessionId, XGI_SESSION_CREATE* data) {
  ...
  netplay_obs::Emit("session_registered", fmt::format("{:016X}", sessionId));
}
